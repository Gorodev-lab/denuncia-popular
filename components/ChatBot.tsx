    ...
    <div className="fixed bottom-24 right-8 w-[350px] h-[500px] bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl flex flex-col z-50 overflow-hidden animate-fade-in-up">
      ...
      <div className="p-4 bg-zinc-900/80 backdrop-blur-md border-b border-zinc-800 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-red-800 flex items-center justify-center shadow-lg shadow-red-900/20">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4" ref={scrollRef}>
        {messages.map((msg) => (
          <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
             <div className={`w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center ${msg.role === 'user' ? 'bg-zinc-800' : 'bg-red-900/20'}`}>
                {msg.role === 'user' ? <User size={14} className="text-zinc-400" /> : <Sparkles size={14} className="text-red-600" />}
             </div>
      ...
      <div className="p-4 bg-zinc-900/50 border-t border-zinc-800">
        <div className="relative">
          <input
            ...
            className="w-full bg-zinc-950 border border-zinc-800 rounded-full pl-4 pr-10 py-3 text-xs text-white focus:ring-1 focus:ring-red-700 outline-none"
          />
          <button 
            onClick={handleSend}
            className="absolute right-1 top-1 bottom-1 w-8 bg-zinc-800 hover:bg-red-800 text-zinc-400 hover:text-white rounded-full flex items-center justify-center transition-colors"
          >
            <Send size={14} />
          </button>
        </div>
