    ...
    <nav className="w-full py-8 px-4" aria-label="Wizard Progress">
      <div className="max-w-4xl mx-auto">
        <ol className="flex justify-between items-center relative">
          ...
          {/* Active Progress Bar */}
          <div 
            className="absolute top-1/2 left-0 h-1 bg-red-800 -z-10 rounded-full transition-all duration-500 ease-out"
              ...
              <li key={s.id} className="flex flex-col items-center gap-2 group" aria-current={isActive ? 'step' : undefined}>
                <div 
                  className={`
                    ...
                    ${isActive 
                      ? 'bg-zinc-950 border-red-700 text-red-600 shadow-[0_0_20px_rgba(204,0,0,0.4)] scale-110' 
                      : isCompleted 
                        ? 'bg-red-800 border-transparent text-white' 
                ...
                <span 
                  className={`
                    ...
                    ${isActive ? 'text-red-600 translate-y-0' : 'text-zinc-600 translate-y-1'}
                  `}
                >
                  {s.label}
                </span>
