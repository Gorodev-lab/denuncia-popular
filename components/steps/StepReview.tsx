...
const SummarySidebar = ({
  draft, isGeneratingPdf, isSubmitting, gdocsMsg,
  onDownloadPDF, onDownloadDOCX, onDownloadMD, onExportGoogleDocs, onSubmit, onBack
}: any) => (
  <div className="lg:w-1/3 flex flex-col justify-center space-y-6">
    <div>
      <h2 className="text-3xl font-bold mb-2 text-white">Revisión Final</h2>
      <p className="text-zinc-400">Verifica los datos antes de generar el documento legal inmutable.</p>
    </div>
    ...
    <div className="hidden lg:block pt-4 space-y-3">
      ...
      <button onClick={onDownloadPDF} disabled={isGeneratingPdf} className="w-full group relative px-8 py-3 rounded-full font-bold text-white overflow-hidden disabled:opacity-80 transition-all border border-zinc-700 hover:bg-zinc-800">
        <span className="relative flex items-center justify-center gap-3">
          {isGeneratingPdf ? <>Generando PDF...</> : <><Printer size={18} className="text-zinc-400 group-hover:text-white transition-colors" />Descargar PDF</>}
        </span>
      </button>

      <button onClick={onDownloadDOCX} className="w-full group relative px-8 py-3 rounded-full font-bold text-white overflow-hidden transition-all border border-zinc-700 hover:bg-zinc-800">
        <span className="relative flex items-center justify-center gap-3">
      ...
      <button onClick={onDownloadMD} className="w-full group relative px-8 py-3 rounded-full font-bold text-white overflow-hidden transition-all border border-zinc-700 hover:bg-zinc-800">
        <span className="relative flex items-center justify-center gap-3">
      ...
      <button onClick={onExportGoogleDocs} className="w-full group relative px-8 py-3 rounded-full font-bold text-white overflow-hidden transition-all border border-blue-800/60 hover:bg-blue-900/30">
        <span className="relative flex items-center justify-center gap-3">
      ...
      <button onClick={onSubmit} disabled={isSubmitting} className="w-full group relative px-8 py-4 rounded-full font-bold text-white overflow-hidden disabled:opacity-80 transition-all shadow-lg shadow-green-900/20">
        ...
        <span className="relative flex items-center justify-center gap-3">
          {isSubmitting ? (
            <><span className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />Firmando...</>
          ) : (
            <><FileCheck size={20} />Firmar y Presentar Digitalmente</>
          )}
        </span>
      </button>

      <button onClick={onBack} disabled={isSubmitting} className="w-full mt-2 text-zinc-500 hover:text-white text-sm font-medium transition-colors uppercase tracking-widest">
        Volver a editar
      </button>
    </div>
  </div>
...
const DocumentPreview = ({ draft }: any) => (
  <div className="lg:w-2/3 bg-zinc-200 text-slate-900 shadow-2xl rounded-sm overflow-hidden relative min-h-[600px] flex flex-col">
    ...
    <div className="p-8 md:p-12 flex-1 font-serif">
      <div className="text-right mb-8 border-b-2 border-slate-900 pb-4">
        ...
        <p className="text-sm font-bold text-slate-900 uppercase mt-1">ASUNTO: DENUNCIA POPULAR</p>
      </div>

      <div className="space-y-6 text-sm md:text-base leading-relaxed text-slate-800">
        <p className="font-bold">
          A LA AUTORIDAD {draft.aiAnalysis?.competency || 'COMPETENTE'}<br />PRESENTE.
        </p>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6 text-xs text-slate-600 border border-slate-300 p-4">
          ...
          <div>
            ...
            <ul className="list-disc list-inside mt-1">
              {draft.evidenceFiles.length > 0 ? draft.evidenceFiles.map((f: File, i: number) => <li key={i}>{f.name}</li>) : <li>Sin adjuntos</li>}
            </ul>
          </div>
        </div>
...
const MobileActions = ({
  gdocsMsg, isGeneratingPdf, isSubmitting,
  onDownloadPDF, onDownloadDOCX, onDownloadMD, onExportGoogleDocs, onSubmit
}: any) => (
  <div className="lg:hidden mt-6 pb-6 space-y-3">
    ...
    <button onClick={onDownloadDOCX} className="w-full group relative px-8 py-3 rounded-full font-bold text-white overflow-hidden transition-all border border-zinc-700 hover:bg-zinc-800">
      <span className="relative flex items-center justify-center gap-3"><FileText size={16} /> Descargar .DOCX</span>
    </button>
    <button onClick={onDownloadMD} className="w-full group relative px-8 py-3 rounded-full font-bold text-white overflow-hidden transition-all border border-zinc-700 hover:bg-zinc-800">
      <span className="relative flex items-center justify-center gap-3"><FileCode size={16} /> Descargar .MD</span>
    </button>
    <button onClick={onExportGoogleDocs} className="w-full group relative px-8 py-3 rounded-full font-bold text-white overflow-hidden transition-all border border-blue-800/60 hover:bg-blue-900/30">
      <span className="relative flex items-center justify-center gap-3"><Globe size={16} className="text-blue-400" /> Exportar a Google Docs</span>
    </button>
    <button onClick={onSubmit} disabled={isSubmitting} className="w-full group relative px-8 py-4 rounded-full font-bold text-white overflow-hidden disabled:opacity-80 transition-all shadow-lg shadow-green-900/20">
      <div className="absolute inset-0 bg-gradient-to-r from-green-600 to-teal-600 transition-all duration-300 group-hover:scale-105"></div>
      <span className="relative flex items-center justify-center gap-3">
